...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 11, 2024 at 06:03 PM
...
--
-- Stand-in structure for view `pierwsza_godzina`
-- (See below for the actual view)
--
CREATE TABLE `pierwsza_godzina` (
`godzina` bigint(2)
,`klasa` char(2)
...
--
-- Stand-in structure for view `pierwsza_godzina_n`
-- (See below for the actual view)
--
CREATE TABLE `pierwsza_godzina_n` (
`godzina` bigint(2)
...
--
DROP TABLE IF EXISTS `pierwsza_godzina`;
...
--
-- Structure for view `pierwsza_godzina_n`
--
DROP TABLE IF EXISTS `pierwsza_godzina_n`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pierwsza_godzina_n`  AS SELECT `g`.`id_g` AS `godzina`, `n`.`imie_nazwisko` AS `nauczyciel`, `p`.`nazwa` AS `przedmiot`, `pl`.`numer_s` AS `sala` FROM (((`godzina` `g` join `plan_lekcji` `pl` on(`g`.`id_g` = `pl`.`id_g`)) join `nauczyciele` `n` on(`n`.`skrot` = `pl`.`skrot`)) join `przedmiot` `p` on(`pl`.`id_p` = `p`.`id_p`)) WHERE `g`.`id_g` = 1 ;
